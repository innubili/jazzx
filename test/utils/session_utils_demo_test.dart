  ...
  group('createRandomDraftSession Demo', () {
    test('demonstrates random draft session creation', () {
      ...
      for (int i = 0; i < sessions.length; i++) {
        ...
        print(
          '  Warmup: ${session.warmup != null ? intSecondsToHHmmss(session.warmup!.time) : 'None'}',
        );
        ...
        for (final entry in session.categories.entries) {
          if (entry.value.time > 0) {
            print(
              '    ${entry.key.name}: ${intSecondsToHHmmss(entry.value.time)} (BPM: ${entry.value.bpm})',
            );
          }
        }
        ...
        print(
          '  Recalculated Duration: ${intSecondsToHHmmss(recalculatedDuration)}',
        );
        print(
          '  Duration Match: ${session.duration == recalculatedDuration ? '✅' : '❌'}',
        );

        final categoriesWithTime =
            session.categories.values.where((cat) => cat.time > 0).length;
        print('  Categories with Time: $categoriesWithTime');
        print(
          '  Has Exactly 2 Categories: ${categoriesWithTime == 2 ? '✅' : '❌'}',
        );
    ...
    test('demonstrates duration ranges', () {
      ...
      print(
        'Short Sessions (6-10 min): ${shortSessions.map((d) => intSecondsToHHmmss(d)).join(', ')}',
      );
      print(
        'Long Sessions (20-180 min): ${longSessions.map((d) => intSecondsToHHmmss(d)).join(', ')}',
      );
      ...
      expect(
        shortSessions.length + longSessions.length,
    ...
    test('demonstrates time allocation ratios', () {
      ...
      for (final entry in categoriesWithTime) {
        ...
        print(
          '${entry.key.name}: ${intSecondsToHHmmss(entry.value.time)} ($percentage% of practice time)',
        );
      }

      // Verify 1/3 and 2/3 allocation
      final times =
          categoriesWithTime.map((e) => e.value.time).toList()..sort();
      ...
      print(
        'Actual allocation: ${times.map((t) => intSecondsToHHmmss(t)).join(' + ')}',
      );
